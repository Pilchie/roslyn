...
{
    /// <summary>
    /// Wraps test cases for FactAttribute so the test case runs on the WPF STA thread
    /// </summary>
    ...
    public class WpfTestCase : LongLivedMarshalByRefObject, IXunitTestCase
    {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    Task.Run(async delegate
                    {
                        try
                        {
                            await testCaseTask.ConfigureAwait(true);
                        }
                        finally
                        {
        ...
        public ISourceInformation SourceInformation
        {
            get
            {
                return _testCase.SourceInformation;
            }

            set
            {
                _testCase.SourceInformation = value;
            }
        }
        ...
        public object[] TestMethodArguments
        {
            get
            {
                return _testCase.TestMethodArguments;
            }
        }
